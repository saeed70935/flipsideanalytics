...
export default function BridgedETHVolumeComp({ className, options, height, CurrentTimeSpan }: Props) {
    const Result: FlipsideResponse = useFlipside(useQueryWithTimeSpan2(Queries.Bridge.BridgedETHVolume, "Last 30 days"));
        ...
        if (Result.Success && Grouped.vertical.length>0) {
            ...
            Grouped.vertical.map((item,index)=>{
                ...
                data : item.data.map(item=>{
                   ...
                   color: index  == 0 ? 'rgb(255, 99, 132)' : "#20c997",
                //    stack: 'a',
                //    emphasis: {
                //        focus: 'series'
                //    }
    ...
    return (
        <>
            {Result.Loading ? <SpinnerLoader height={height} className={'ht-250'} /> :
                <div  >
